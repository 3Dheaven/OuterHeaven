...
layout (binding=0) uniform sampler2D grassTex1;
layout (binding=1) uniform sampler2D grassTex2;
layout (binding=2) uniform sampler2D grassTex3;
layout (binding=3) uniform sampler2D grassTex4;

flat in int choiceTex;
...
void main()
{
	...
	if(choiceTex == 0)
		colorTex = texture2D(grassTex1, TexCoords);
	else if(choiceTex == 1)
		colorTex = texture2D(grassTex2, TexCoords);
	else if(choiceTex == 2)
		colorTex = texture2D(grassTex3, TexCoords);
	else
		colorTex = texture2D(grassTex4, TexCoords);
...
vec3 ambientColor(Light light)
{
	vec3 ambient, colorTex;

	if(choiceTex == 0)
		ambient = texture2D(grassTex1, TexCoords).xyz * light.ambient * vec3(0.04);
	else if(choiceTex == 1)
		ambient = texture2D(grassTex2, TexCoords).xyz * light.ambient * vec3(0.04);
	else if(choiceTex == 2)
		ambient = texture2D(grassTex3, TexCoords).xyz * light.ambient * vec3(0.04);
	else
		ambient = texture2D(grassTex4, TexCoords).xyz * light.ambient * vec3(0.04);
...
{
	vec3 diffuse, colorTex;

	if(choiceTex == 0)
		diffuse = max(dot(L,normal),0) * texture2D(grassTex1, TexCoords).xyz * light.diffuse;
	else if(choiceTex == 1)
		diffuse = max(dot(L,normal),0) * texture2D(grassTex2, TexCoords).xyz * light.diffuse;
	else if(choiceTex == 2)
		diffuse = max(dot(L,normal),0) * texture2D(grassTex3, TexCoords).xyz * light.diffuse;
	else
		diffuse = max(dot(L,normal),0) * texture2D(grassTex4, TexCoords).xyz * light.diffuse;
