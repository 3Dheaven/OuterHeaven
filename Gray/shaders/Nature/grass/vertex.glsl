...
void main()
{
	seeding(morton2D(gl_InstanceID / 3, 1));

	float x = uniform_0_1()*10;
	float z = uniform_0_1()*4;

	switch(gl_InstanceID % 3)
	{
		case 0:
			gl_Position = projection * view * vec4(vec3(position.x + x, position.y, position.z + z), 1.0f);
			break;

		case 1:
			gl_Position = projection * view * (vec4(x, 0.0, z, 0.0f) + (rotationMatrix1 * vec4(position, 1.0f)));
			break;

		case 2:
			gl_Position = projection * view * (vec4(x, 0.0, z, 0.0f) + (rotationMatrix2 * vec4(position, 1.0f)));
			break;
	}

	N = vec4(transpose(inverse(view)) * vec4(0.0, 1.0, 0.0, 1.0f)).xyz;

	choiceTex = gl_InstanceID % 4;

    TexCoords = texCoords;

	fragPos = vec4( view * vec4(position, 1.0f) ).xyz;
}